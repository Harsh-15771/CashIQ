...
export default function ActionQueueTab({ queue, auditTrail, onActionCompleted, onReviewAction }) {
          ...
          <table className="w-full text-left text-[12px]">
            ...
            <tbody className="font-mono">
              {auditTrail.map((entry, idx) => (
                <tr
                  key={idx}
                  className={`border-b border-white/[0.02] hover:bg-white/[0.02] transition-colors ${entry.security_event ? '' : ''
                    }`}
